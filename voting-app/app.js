/**
 * MVP Voting Website — 0–10 scoring, one vote per artist per IP
 * Stack: Node.js 18+, Express, better-sqlite3, Helmet, nanoid
 *
 * Features
 * - Artist signup (creates a shareable voting link)
 * - Voters score 0..10 (integer) via a simple mobile-friendly page
 * - Enforces one vote per artist per IP (hashed, salted) + soft cookie guard
 * - Live leaderboard (average score desc, tie-breaker by votes count)
 * - Basic admin JSON endpoints
 *
 * Quick start
 *   1) npm init -y
 *   2) npm i express better-sqlite3 helmet nanoid cookie-parser
 *   3) node app.js
 *   4) Open http://localhost:3000
 *
 * NOTE: IP limits are only a basic anti-duplicate measure. For production,
 *       consider adding: email/OTP, device fingerprint, ReCAPTCHA/HCaptcha,
 *       and proxy/VPN detection.
 */
...
app.get('/', (req, res) => {
  ...
  res.send(layout({ title: 'Voting — Home', body }));
});
